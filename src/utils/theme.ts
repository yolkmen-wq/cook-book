...
export const themeColors = reactive<ThemeConfig['colors']>({ ...themes.light.colors })
...
export class ThemeManager {
  ...
  private updateMiniProgramTheme(theme: ThemeConfig) {
    ...
    try {
      uni.setNavigationBarColor({
        frontColor: this.isLightColor(theme.colors.background) ? '#000000' : '#ffffff',
        backgroundColor: theme.colors.background,
        animation: {
          duration: 300,
          timingFunc: 'easeIn'
  ...
  private updateTabBarTheme(theme: ThemeConfig) {
    try {
      uni.setTabBarStyle({
        ...
        backgroundColor: theme.colors.background,
        borderStyle: this.isLightColor(theme.colors.background) ? 'black' : 'white'
      })
    } catch (error) {
  ...
  private updateStatusBarColor(backgroundColor: string) {
    ...
    metaAppleStatusBar.setAttribute('content', this.isLightColor(backgroundColor) ? 'default' : 'black-translucent')
  }
