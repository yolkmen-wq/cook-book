<template>
  ...
  >
    ...
    <slot name="left">
      <text
        v-if="_attrs.leftText !== ''"
        :style="{ marginRight: '24rpx', fontSize: _fontSize }"
        >{{ _attrs.leftText }}</text
      >
    </slot>
    ...
    >
      ...
      <slot name="leftIcon">
        <tm-icon
          v-if="_attrs.leftIcon"
          :size="_attrs.fontSize"
          _style="margin-left:24rpx"
          :name="leftIcon"
          :color="_attrs.iconColor || config.color"
        ></tm-icon>
      </slot>
      <view
        class="tmInputWrapBox"
        :style="[
          _attrs.type != 'textarea'
            ? { height: _minHeight }
            : { minHeight: _minHeight },
        ]"
      >
        ...
        <input
          ...
          :value="nowValue"
          @change="onchange"
          :password="_attrs.password && showPasswordEye"
          :disabled="_attrs.disabled"
          :maxlength="_attrs.maxlength"
          :cursorSpacing="_attrs.cursorSpacing"
          :focus="_attrs.focus"
          :confirmType="_attrs.confirmType"
          :confirmHld="_attrs.confirmHold"
          :type="_attrs.type"
          :cursorColor="_currorColor"
          :selectionStart="_attrs.selectionStart"
          :selectionEnd="_attrs.selectionEnd"
          :adjustPosition="_attrs.adjustPosition"
          :autoBlur="_attrs.autoBlur"
          :ignoreCompositionEvent="_attrs.ignoreCompositionEvent"
          :holdKeyboard="_attrs.holdKeyboard"
          :safePasswordCertPath="_attrs.safePasswordCertPath"
          :safePasswordLength="_attrs.safePasswordLength"
          :safePasswordTimeStamp="_attrs.safePasswordTimeStamp"
          :safePasswordNonce="_attrs.safePasswordNonce"
          :safePasswordSalt="_attrs.safePasswordSalt"
          :safePasswordCustomHash="_attrs.safePasswordCustomHash"
          :randomNumber="_attrs.randomNumber"
          :controlled="_attrs.controlled"
          :alwaysSystem="_attrs.alwaysSystem"
          :inputmode="_attrs.inputmode"
          class="tmInputInput tmInputText"
          :placeholderStyle="
            `textAlign: ${_attrs.align};fontSize:${_fontSize};` +
            _attrs.placeholderStyle
          "
          :placeholder="_attrs.placeholder"
          :placeholderClass="_attrs.placeholderClass"
          :style="[
            {
        ...
        <textarea
          ...
          :autoHeight="_attrs.autoHeight"
          :fixed="_attrs.fixed"
          :showConfirmBar="_attrs.showConfirmBar"
          :value="nowValue"
          :disabled="_attrs.disabled"
          :maxlength="_attrs.maxlength"
          :cursorSpacing="_attrs.cursorSpacing"
          ...
          :placeholder="_attrs.placeholder"
          :placeholderClass="_attrs.placeholderClass"
          class="tmInputInput tmInputTextArea"
          :style="[
            {
              width: 'auto',
              textAlign: _attrs.align,
        ...
        <view
          class="tmInputShowChar"
          v-if="_attrs.maxlength > -1 && _attrs.showCharCount"
        >
          <!-- 
                    @slot 输入框右侧显示字数
                    -->
      ...
      <tm-icon
        :size="_attrs.fontSize"
        @click="onclear"
        _style="padding:16rpx;"
        :color="_clearColor"
        v-if="_attrs.showClear"
      ...
      <tm-icon
        ...
        _style="padding:16rpx;"
        :color="showPasswordEye ? _clearColor : config.color"
        :name="showPasswordEye ? 'eye-off-line' : 'eye-line'"
      ></tm-icon>
      <!-- 
            @slot 输入框内右侧图标
            -->
      <slot name="rightIcon">
        <tm-icon
          :size="_attrs.fontSize"
          v-if="_attrs.rightIcon"
          :_style="_attrs.showClear ? 'margin-right:24rpx' : 'margin:0 24rpx'"
          :name="rightIcon"
          :color="_attrs.iconColor || config.color"
        ></tm-icon>
      </slot>
    ...
    <slot name="right">
      <text
        v-if="_attrs.rightText != ''"
        :style="{ marginLeft: '24rpx', fontSize: _fontSize }"
        >{{ _attrs.rightText }}</text
      >
    </slot>
  </view>
</template>
...
const _focusHighlight = computed(() => {
  ...
  return attrs.focusHighlight
    ? attrs.focusHighlightStyle ||
        `border:solid 2px ${getDefaultColor(config.color)} !important`
    : defaultsolid;
});
...
.tmInputWrapBox {
  ...
  .tmInputPlaceholder {
    ...
    text-wrap: nowrap;
    opacity: 0;
    transition: all 0.2s cubic-bezier(0.42, 0.38, 0.15, 0.93);
